...
const std::map<std::string, double>& SearchServer::GetWordFrequencies(int document_id) const {
    if (documents_words_frequencies_.at(document_id).empty()) {
        return EMPTY_MAP;
    }
    return documents_words_frequencies_.at(document_id);
}

void SearchServer::RemoveDocument(int document_id) {
    documents_.erase(documents_.find(document_id));
    auto iter_del = document_ids_.begin();
    int i = 0;
    for (auto doc_id : document_ids_) {
        if (doc_id == document_id) {
            iter_del += i;
        }
        i++;
    }
    document_ids_.erase(iter_del);
}