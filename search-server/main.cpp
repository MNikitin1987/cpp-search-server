...
public: 
    ...
    int GetDocumentId(int index) const {
        if(index < 0 || index > ids_.size()) {
            throw out_of_range("Выходит за пределы допустимого диапазона"s);
        }
        return ids_[index];
    }
...
// Ответ на ваш комментарий: "при работе с отдельными словами (после обрезания первого минуса), проверка на второй минус сводится к проверке нулевого элемента слова, так что не нужна здесь сложная логика. После обрезания первого минуса в слове - надо сделать две проверки, что слово не пустым стало  и что в нулевом элементе не минус"
// Функция IsExtraDash используется в нескольких местах, убрав из неё проверку двух последовательных элементов (т.е. сохранив её только для проверки слов) мы уберём универсальность. Я ещё немного упростил функуию, прошу принять в таком виде.
